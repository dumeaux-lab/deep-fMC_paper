create.bresat.heatmap <- function(bs, cl.df, cl.var, cl.height=8, title=title)
{
  ...
  data = bs$dat
  mclinical = cl.df[sub.bs$pat.order,cl.var]
  ...
  if(length(sub.bs$dn)>0){
    up.dn[(length(sub.bs$up)+1):(length(sub.bs$gene.order))] = -1}
...
###                 region exists).
heatmap.color.scheme <- function(low.breaks=seq(-1.5, 0, length.out=21),
                                 high.breaks=seq(0, 1.5, length.out=21),
                                 masked.color="black",
...
###     col.grid        color used for grid lines in heatmap
###
### Returns:
###
...
###     lwd                 line width relative to the default (default=0.01)
