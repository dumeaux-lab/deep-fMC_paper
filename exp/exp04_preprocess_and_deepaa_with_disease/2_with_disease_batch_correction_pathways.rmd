...
```{r}
pathways <- read.csv(paste(path_main, "/filtered_paths_with_disease_samples_pathway_counts.csv", sep = ""), header = TRUE, row.names = 1, sep = ",")
batch<-read.csv(paste(path_main,"/filtered_paths_with_disease_samples_metadata.csv", sep = ""), row.names = 1)
...
write.csv(batch_filtered, file = paste(path_main, '/batchfix_paths_with_disease_samples_pathway_metadata.csv', sep = ""))
...
```
```{r}
write.csv(adjusted, file = paste(path_main, '/batchfix_paths_with_disease_samples_pathway_counts.csv', sep = ""))
```
